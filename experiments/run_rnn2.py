...
def create_pretrained_embedding_emoji(lookup, vocab, used_emojis, dims):
    ...
    for emoji_ in used_emojis:
        new_lookup[emoji_] = np.concatenate((np.zeros(dims), emoji_score(emoji_))) 
        #new_lookup[emoji_] = np.concatenate((emoji_sub(emoji_, lookup, dims), emoji_score(emoji_))) 

    for key in lookup.keys():
        if key not in used_emojis:
            new_lookup[key] = np.concatenate((lookup[key], np.array([0.])))
...
pickle.dump(results, open("results_bkp2.pkl", "wb"))
